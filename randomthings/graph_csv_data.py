...
def parse_csv_data(csv_file):
    """Parse the CSV data and extract network information"""
    # Read the CSV file
    ...
    # Parse the data (assuming the structure from your CSV)
    omega_str = row.iloc[-3]  # Second to last column contains omega
    edges_str = row.iloc[-2]  # Third to last column contains edges
    metadata_str = row.iloc[-1]  # Last column contains metadata
